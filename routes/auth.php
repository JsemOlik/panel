...
use Pterodactyl\Http\Middleware\RequirePasswordLogin;
...
Route::middleware(['throttle:authentication'])->group(function () {
    // Login endpoints.
    Route::post('/login', [Auth\LoginController::class, 'login'])->middleware([RequirePasswordLogin::class, 'recaptcha']);
    ...
    Route::post('/password', [Auth\ForgotPasswordController::class, 'sendResetLinkEmail'])
        ...
        ->middleware([RequirePasswordLogin::class, 'recaptcha']);

    // OAuth endpoints. These are also used by signed-in users linking a provider to their account.
...
Route::get('/oauth/{provider}/logo', [Auth\OAuthController::class, 'logo'])
    ->withoutMiddleware('guest')
    ->name('auth.oauth.logo');

// Password reset routes. This endpoint is hit after going through
// the forgot password routes to acquire a token (or after an account
// is created).
Route::post('/password/reset', Auth\ResetPasswordController::class)
    ->middleware(RequirePasswordLogin::class)
